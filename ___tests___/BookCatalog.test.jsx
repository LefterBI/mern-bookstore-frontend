...
describe('BookCatalog – Functionalitati de baza', () => {
  ...
  const mockProduct = {
    ...
    price: 39.99,
    description:
      'Ghidul definitiv pentru MongoDB – de la concepte de bază la arhitecturi avansate. Ideal pentru dezvoltatori care vor să stăpânească această bază de date NoSQL populară.',
    category: 'MongoDB',
    imageUrl: 'https://m.media-amazon.com/images/I/91hgdbExtlL._SL1500_.jpg',
    stock: 25,
    specifications: {
...
const waitForLoadingToComplete = async () => {
  await waitFor(() => {
    expect(
      screen.queryByText('Se încarcă produsele...')
...
it('ar trebui sa afiseze fara erori si sa incarce produsele', async () => {
  ...
  expect(
    screen.getByText('Se încarcă produsele...')
  ).toBeInTheDocument();

  // Așteaptă și verifică încărcarea completă
  ...
  expect(
    screen.getByText('MERN BookStore')
  ).toBeInTheDocument();
  expect(
...
it('ar trebui sa afiseze functionalitatea de cautare', async () => {
  ...
  // Verifică elementele interfeței de căutare
...
it('ar trebui sa afiseze starea de loading initial', async () => {
  ...
  // Rezolvă promisa și așteaptă finalizarea
  resolvePromise();
  await waitForLoadingToComplete();
